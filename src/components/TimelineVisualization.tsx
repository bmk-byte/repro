  ...
  return (
    <Card>
      <div className="flex justify-between items-center mb-6">
        ...
        >
          <option value="all">{t('timeline.allCountries')}</option>
      ...
      {loading ? (
        ...
        <>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={data}>
                ...
                <Area
                  ...
                  dataKey="ongoing"
                  stackId="1"
                  name={t('timeline.legend.ongoing')}
                ...
                <Area
                  ...
                  dataKey="resolved"
                  stackId="1"
                  name={t('timeline.legend.resolved')}
                ...
                <Area
                  ...
                  dataKey="dismissed"
                  stackId="1"
                  name={t('timeline.legend.dismissed')}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            <div className="bg-stone-50 p-4 rounded-lg">
              <Flex>
                ...
                <Text className="font-medium">{t('timeline.stats.filedCases')}</Text>
              </Flex>
              <Text className="mt-2 text-2xl font-bold">
                {data.reduce((sum, item) => sum + item.filed, 0)}
              </Text>
              <Text className="text-stone-500 text-sm">{t('timeline.stats.filedCasesSubtitle')}</Text>
            </div>

            <div className="bg-stone-50 p-4 rounded-lg">
              <Flex>
                <Clock className="h-5 w-5 text-info" />
                <Text className="font-medium">{t('timeline.stats.ongoingCases')}</Text>
              </Flex>
              <Text className="mt-2 text-2xl font-bold">
                {data.reduce((sum, item) => sum + item.ongoing, 0)}
              </Text>
              <Text className="text-stone-500 text-sm">{t('timeline.stats.ongoingCasesSubtitle')}</Text>
            </div>

            <div className="bg-stone-50 p-4 rounded-lg">
              <Flex>
                <CheckCircle className="h-5 w-5 text-success" />
                <Text className="font-medium">{t('timeline.stats.resolvedCases')}</Text>
              </Flex>
              <Text className="mt-2 text-2xl font-bold">
                {data.reduce((sum, item) => sum + item.resolved, 0)}
              </Text>
              <Text className="text-stone-500 text-sm">{t('timeline.stats.resolvedCasesSubtitle')}</Text>
            </div>

            <div className="bg-stone-50 p-4 rounded-lg">
              <Flex>
                <XCircle className="h-5 w-5 text-danger" />
                <Text className="font-medium">{t('timeline.stats.dismissedCases')}</Text>
              </Flex>
              <Text className="mt-2 text-2xl font-bold">
                {data.reduce((sum, item) => sum + item.dismissed, 0)}
              </Text>
              <Text className="text-stone-500 text-sm">{t('timeline.stats.dismissedCasesSubtitle')}</Text>
            </div>
          </div>
          ...
          <div className="mt-6 p-4 bg-stone-50 rounded-lg">
            <Flex>
              <div>
                <Text className="font-medium">{t('timeline.avgResolutionTime.title')}</Text>
                <Text className="mt-1 text-2xl font-bold">{t('timeline.avgResolutionTime.value', { count: averageResolutionTime })}</Text>
                <Text className="text-stone-500 text-sm">{t('timeline.avgResolutionTime.subtitle')}</Text>
              </div>
              <div className="text-right">
                <Text className="font-medium">{t('timeline.caseAging.title')}</Text>
                <div className="flex items-center mt-1 justify-end">
                  <div className="w-2 h-2 rounded-full bg-success mr-1"></div>
                  <Text className="text-sm">{t('timeline.caseAging.lessThan30', { value: caseAging.lessThan30 })}</Text>
                </div>
                <div className="flex items-center justify-end">
                  <div className="w-2 h-2 rounded-full bg-warning mr-1"></div>
                  <Text className="text-sm">{t('timeline.caseAging.between30And90', { value: caseAging.between30And90 })}</Text>
                </div>
                <div className="flex items-center justify-end">
                  <div className="w-2 h-2 rounded-full bg-danger mr-1"></div>
                  <Text className="text-sm">{t('timeline.caseAging.moreThan90', { value: caseAging.moreThan90 })}</Text>
                </div>
              </div>
            </Flex>
          </div>
