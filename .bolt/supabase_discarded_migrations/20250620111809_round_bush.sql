...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'resources') THEN
    ...
    CHECK (resource_type IN ('template', 'guide', 'analysis', 'research'));
  END IF;
END $$;
...
-- Fix the type mismatch by using proper casting
CREATE POLICY "Users can delete their own resources"
ON storage.objects FOR DELETE
TO authenticated
USING (bucket_id = 'resources' AND auth.uid()::text = owner);
...
-- Create an ALL policy for users to manage their own resources
...
-- Create individual policies for update and delete operations
